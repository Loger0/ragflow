...
# ADAPT: PostgreSQL Text type replaces MySQL LONGTEXT
COLUMN_DEFINITIONS: list[Column] = [
    Column("id", Text, primary_key=True, comment="unique record id"),
    Column("message_id", Text, nullable=False, comment="message id"),
    Column("message_type_kwd", Text, nullable=True, comment="message type"),
    Column("source_id", Text, nullable=True, comment="source message id"),
    Column("memory_id", Text, nullable=False, comment="memory id"),
    Column("user_id", Text, nullable=True, comment="user id"),
    Column("agent_id", Text, nullable=True, comment="agent id"),
    Column("session_id", Text, nullable=True, comment="session id"),
    Column("zone_id", Integer, nullable=True, default=0, comment="zone id"),
    Column("valid_at", Text, nullable=True, comment="valid at timestamp string"),
    Column("invalid_at", Text, nullable=True, comment="invalid at timestamp string"),
    Column("forget_at", Text, nullable=True, comment="forget at timestamp string"),
    Column("status_int", Integer, nullable=False, default=1, comment="status: 1 for active, 0 for inactive"),
    Column("content_ltks", Text, nullable=True, comment="content with tokenization"),
    Column("tokenized_content_ltks", Text, nullable=True, comment="fine-grained tokenized content"),
]
...
class VBConnection(VastbaseConnectionBase):
    """Vastbase document store connection for message/memory storage.

    Inherits from VastbaseConnectionBase. Implements search (fusion/vector/fulltext/
    filter paths), insert, update, and field mapping for message data.
    """

    def __init__(self):
        super().__init__(logger_name='ragflow.memory_vastbase_conn')
    ...
    def _get_vector_column_name_from_table(self, table_name: str) -> Optional[str]:
        ...
        Returns:
            Vector column name or None.
        """
        # ADAPT: PostgreSQL information_schema.columns + regex replaces MySQL REGEXP
        sql = (
            "SELECT column_name"
            "  FROM information_schema.columns"
            "  WHERE table_schema = 'public'"
            "    AND table_name = %s"
            "    AND column_name ~ '^q_[0-9]+_vec$'"
            "  LIMIT 1"
        )
        try:
    ...
    def convert_field_name(field_name: str, use_tokenized_content: bool = False) -> str:
        ...
        Args:
            ...
            use_tokenized_content: If True, use fine-grained tokenized content field.
    ...
    def map_message_to_vb_fields(message: dict) -> dict:
        """Map message dictionary fields to Vastbase document fields.

        Args:
            message: Message dictionary.

        Returns:
            Vastbase document dictionary.
        """
        storage_doc = {
            "id": message.get("id"),
    ...
    def get_message_from_vb_doc(doc: dict) -> dict:
        """Convert a Vastbase document back to a message dictionary.

        Args:
            doc: Vastbase document dict.

        Returns:
            Message dictionary.
        """
        embd_field_name = next(
            (key for key in doc.keys() if re.match(r"q_\d+_vec", key)), None
        )
    ...
    def search(
        ...
        Args:
            ...
            index_names: Index/table names.
            memory_ids: Memory IDs to filter.
            agg_fields: Aggregation fields (unused in memory search).
            rank_feature: Rank feature (unused in memory search).
            hide_forgotten: If True, exclude forgotten messages.

        Returns:
            Tuple of (SearchResult, total_count).
        """
        ...
        fields_expr = ", ".join([f'"{f}"' for f in db_output_fields])
        ...
        for m in match_expressions:
            if isinstance(m, MatchTextExpr):
                ...
                vector_similarity_threshold = (
                    m.extra_options.get("similarity", 0.0) if m.extra_options else 0.0
                )
        ...
        for index_name in index_names:
            ...
            if search_type == "fusion":
                ...
                # ADAPT: PostgreSQL CTE with standard LIMIT/OFFSET
                score_expr = (
                    f"(relevance * {1 - vector_similarity_weight} + "
                    f"{vector_search_score_expr} * {vector_similarity_weight})"
                )
                fusion_sql = (
                ...
                self.logger.debug("MemoryVBConnection.search with fusion sql: %s", fusion_sql)
                rows, elapsed_time = self._execute_search_sql(fusion_sql)
                self.logger.info(
                    f"MemoryVBConnection.search table {table_name}, search type: fusion, "
                    f"elapsed time: {elapsed_time:.3f}s, rows: {len(rows)}"
                )

                for row in rows:
                    result.messages.append(
                        self._row_to_entity(row, db_output_fields + ["_score"])
                    )
                    result.total += 1

            elif search_type == "vector":
                # ADAPT: Standard LIMIT/OFFSET
                ...
                self.logger.debug("MemoryVBConnection.search with vector sql: %s", vector_sql)
                rows, elapsed_time = self._execute_search_sql(vector_sql)
                self.logger.info(
                    f"MemoryVBConnection.search table {table_name}, search type: vector, "
                    f"elapsed time: {elapsed_time:.3f}s, rows: {len(rows)}"
                )
                ...
                self.logger.debug("MemoryVBConnection.search with fulltext sql: %s", fulltext_sql)
                rows, elapsed_time = self._execute_search_sql(fulltext_sql)
                self.logger.info(
                    f"MemoryVBConnection.search table {table_name}, search type: fulltext, "
                    f"elapsed time: {elapsed_time:.3f}s, rows: {len(rows)}"
                )
                ...
                self.logger.debug("MemoryVBConnection.search with filter sql: %s", filter_sql)
                rows, elapsed_time = self._execute_search_sql(filter_sql)
                self.logger.info(
                    f"MemoryVBConnection.search table {table_name}, search type: filter, "
                    f"elapsed time: {elapsed_time:.3f}s, rows: {len(rows)}"
                )
    ...
    def get_forgotten_messages(self, select_fields: list[str], index_name: str,
                               memory_id: str, limit: int = 512):
        ...
        Returns:
            SearchResult with forgotten messages.
        """
        ...
        sql = (
            ...
            f"  FROM \"{index_name}\""
            f"  WHERE \"memory_id\" = %s AND \"forget_at\" IS NOT NULL"
            f"  ORDER BY \"forget_at\" ASC"
            f"  LIMIT {limit}"
        )
        self.logger.debug("MemoryVBConnection.get_forgotten_messages sql: %s", sql)

        rows = self._execute_sql(sql, (memory_id,))
    ...
    def get_missing_field_message(self, select_fields: list[str], index_name: str,
                                  memory_id: str, field_name: str, limit: int = 512):
        ...
        Args:
            ...
            index_name: Table name.
            memory_id: Memory identifier.
            field_name: Field name to check for NULL.
            limit: Maximum number of messages to return.

        Returns:
            SearchResult with messages missing the field.
        """
        if not self._check_table_exists_cached(index_name):
            return None
        ...
        sql = (
            f"SELECT {fields_expr}"
            f"  FROM \"{index_name}\""
            f"  WHERE \"memory_id\" = %s AND \"{db_field_name}\" IS NULL"
            f"  ORDER BY \"valid_at\" ASC"
            f"  LIMIT {limit}"
        )
        self.logger.debug("MemoryVBConnection.get_missing_field_message sql: %s", sql)
    ...
    def insert(self, documents: list[dict], index_name: str, memory_id: str = None) -> list[str]:
        ...
        Args:
            documents: List of message dicts.
        ...
        self.logger.debug("MemoryVBConnection.insert messages: %s", ids)

        res = []
        try:
            self._batch_upsert(index_name, docs)
        except Exception as e:
            self.logger.error(f"MemoryVBConnection.insert error: {str(e)}")
            res.append(str(e))
        return res
    ...
    def update(self, condition: dict, new_value: dict, index_name: str, memory_id: str) -> bool:
        ...
        if "content_ltks" in update_dict:
            update_dict["tokenized_content_ltks"] = fine_grained_tokenize(
                tokenize(update_dict["content_ltks"])
            )
        ...
        self.logger.debug("MemoryVBConnection.update sql: %s", update_sql)

        try:
            self._execute_sql(update_sql)
            return True
        except Exception as e:
            self.logger.error(f"MemoryVBConnection.update error: {str(e)}")
        return False
    ...
    def get_fields(self, res, fields: list[str]) -> dict[str, dict]:
        ...
        Returns:
            Dict mapping message IDs to field dicts.
        """
        if isinstance(res, tuple):
            res = res[0]
        ...
        for doc in messages:
            ...
            for n, v in message.items():
                ...
                if isinstance(v, list):
                    m[n] = v
                    continue
                if n in ["message_id", "source_id", "valid_at", "invalid_at",
                          "forget_at", "status"] and isinstance(v, (int, float, bool)):
                    m[n] = v
                    continue
                if not isinstance(v, str):
                    ...
                    continue
                m[n] = v
            res_fields[doc.get("id", "")] = m
        return res_fields

    def get_highlight(self, res, keywords: list[str], field_name: str):
        """Get highlights from search result.

        Args:
            res: Search result.
            keywords: List of keywords.
            field_name: Field name to highlight.

        Returns:
            Dict mapping message IDs to highlight dicts.
        """
        if isinstance(res, tuple):
            res = res[0]

        if not hasattr(res, 'messages'):
            return get_highlight_from_messages(res, keywords, field_name)

        messages = [self.get_message_from_vb_doc(doc) for doc in res.messages]
        return get_highlight_from_messages(messages, keywords, field_name)

    def get_aggregation(self, res, field_name: str):
        """Get aggregation from search result.

        Args:
            res: Search result.
            field_name: Field name to aggregate.
        ...
        messages = (
            [self.get_message_from_vb_doc(doc) for doc in res.messages]
            if hasattr(res, 'messages') else []
        )
